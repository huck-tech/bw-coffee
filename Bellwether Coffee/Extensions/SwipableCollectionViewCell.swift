...
//  Bellwether Coffee
//
//  Created by Gabriel Pierannunzi on 7/9/18.
 ...
 */
class SwipeableCollectionViewCell: CollectionViewCell {
    ...
    open var swipeableFrame: CGRect {
        set { super.frame = state.isActive ? CGRect(origin: CGPoint(x: swipeableFrame.minX, y: newValue.minY), size: newValue.size) : newValue }
    ...
    func configure() {
        ...
        swipeController = SwipeController(swipeable: self as! UIView & Swipeable, actionsContainerView: contentView)
    ...
    func contains(point: CGPoint) -> Bool {
        return swipeableFrame.contains(point)
    }
    ...
    override open var layoutMargins: UIEdgeInsets {
        get {
            return swipeableFrame.origin.x != 0 ? swipeController.originalLayoutMargins : super.layoutMargins
        }
        set {
...
extension SwipeableCollectionViewCell: SwipeControllerDelegate {
